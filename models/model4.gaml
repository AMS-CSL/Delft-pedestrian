...
global {   
	...
	file shape_file_roads  <- file("../includes/delft_network.shp") ;

	//load shapefile with starting points of pedestrians (points)
	file shape_file_sources <- file("../includes/delft_sources.shp") ;
	...
	file shape_file_pois <- file("../includes/testjunctions.shp") ;
	//load shape_file with boundingbox of the network shapefile
	
	file shape_file_bounds <- file("../includes/delft_boundary.shp") ;
	...
	init {  
		//create road species by from the network shapefile an load the attribute names into corresponding attribute of the road specie
...
entities {
	...
	species source {
		int id;
		//slowly release of pedestrians from the parking: 1 at every tick till capacity is reached
		reflex release_pedestrian when: (numberReleasedPedestrians < capacity) {
				if flip(probabilityOfCreation){
					create people number: 1  {
						speed <- 1.2;
						location <- poi closest_to(one_of(source));
	...
	species people skills: [driving]  { 
		...
		//als simulatietijd < dan beschikbare tijd
...
experiment delft_pedestrian_movement type: gui {
	...
	parameter "Probability of creation of a pedestrian:" var: probabilityOfCreation category: "Control Parameters";
	output {
